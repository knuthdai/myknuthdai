	...
	<?php if ( have_comments() ) : ?>
		...
		<div class="comments">
			...
			<h2 class="comments-title">
				...
				echo _n( 'Comment' , 'Comments' , count($wp_query->comments_by_type['comment']), 'wilson' ); ?>
	...
	<?php $comments_args = array(
		...
		'fields' => apply_filters( 'comment_form_default_fields', array(
			...
			'author' =>
				...
				'<input id="author" name="author" type="text" placeholder="' . __('Name','wilson') . '" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" />' . '<label for="author">Author</label> ' . ( $req ? '<span class="required">*</span>' : '' ) . '</p>',
			...
			'url' =>
			'<p class="comment-form-url">' . '<input id="url" name="url" type="text" placeholder="' . __('Website','wilson') . '" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /><label for="url">Website</label></p>')
		),
	);
